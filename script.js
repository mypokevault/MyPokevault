...
function doSearch() {
  ...
  var url = 'https://api.pokemontcg.io/v2/cards?q=name:' + encodeURIComponent(q) + '&pageSize=24&orderBy=-set.releaseDate';
  fetch(url)
    .then(function(r) {
    ...
    .catch(function(e) {
      el.innerHTML = '<div class="em"><div class="ei">⚠️</div><p>Impossible de joindre l\'API Pokémon TCG.<br>Vérifiez votre connexion ou utilisez la <strong>saisie manuelle</strong>.</p><p style="margin-top:8px;font-size:.76rem;color:var(--t2)">' + esc(e.message) + '</p></div>';
    });
}

function renderResults(cards) {
  var el = document.getElementById('results');
  if (!cards.length) { el.innerHTML = '<div class="em"><div class="ei">🃏</div><p>Aucune carte trouvée.</p></div>'; return; }
  var html = '<div class="rg">';
  for (var i = 0; i < cards.length; i++) {
    var c = cards[i];
    var cm  = c.cardmarket && c.cardmarket.prices;
    var tcp = c.tcgplayer  && c.tcgplayer.prices;
    var avg   = (cm && cm.averageSellPrice) || (tcp && tcp.holofoil && tcp.holofoil.market) || (tcp && tcp.normal && tcp.normal.market) || 0;
    var low   = (cm && cm.lowPrice) || (tcp && tcp.holofoil && tcp.holofoil.low) || (tcp && tcp.normal && tcp.normal.low) || 0;
    var trend = (cm && cm.trendPrice) || 0;
    var img   = (c.images && c.images.small) || '';
    var idx   = window._pendingCards ? window._pendingCards.length : 0;
    if (!window._pendingCards) window._pendingCards = [];
    window._pendingCards.push({card: c, price: avg});
    html += '<div class="rc">';
    ...
    html += '<div class="rb">';
    html += '<div class="rn">' + esc(c.name) + '</div>';
    html += '<div class="rset">' + esc((c.set && c.set.name) || '') + ' · ' + esc(c.rarity || '') + '</div>';
    html += '<div class="pr"><span class="pl">💶 Cote</span><span class="pa">' + (avg ? avg.toFixed(2) + ' €' : '—') + '</span></div>';
    if (low)   html += '<div class="pr"><span class="pl">📉 Min</span><span class="plo">' + low.toFixed(2) + ' €</span></div>';
    if (trend) html += '<div class="pr"><span class="pl">📈 Tendance</span><span class="ptr">' + trend.toFixed(2) + ' €</span></div>';
    html += '</div></div>';
  }
  html += '</div>';
  el.innerHTML = html;
...
// Stocker les cartes en attente pour éviter les problèmes d'encoding dans onclick
...
function pushCard(card) {
  ...
  else { collection.push({ id: collection.length + 1 + '_' + Date.now(), addedAt: new Date().toISOString(), name: card.name, set: card.set, rarity: card.rarity, rarityLabel: card.rarityLabel, qty: card.qty, value: card.value, condition: card.condition, img: card.img }); }
  save();
...
function removeCard(id) {
  var next = [];
  for (var i = 0; i < collection.length; i++) { if (String(collection[i].id) !== String(id)) next.push(collection[i]); }
  collection = next;
  save();
  renderColl();
  renderDash();
...
function renderDash() {
  ...
  var tQ = 0; for (var i = 0; i < collection.length; i++) tQ += collection[i].qty;
  var tV = 0; for (var i = 0; i < collection.length; i++) tV += collection[i].value * collection[i].qty;
  var avg = tQ ? tV / tQ : 0;
  ...
  document.getElementById('stats-grid').innerHTML =
    ...
    sc('💶 Valeur totale', tV.toFixed(0) + ' €', '~' + avg.toFixed(2) + ' € / carte') +
    sc('⭐ Plus précieuse', top ? top.value.toFixed(2) + ' €' : '—', top ? esc(top.name) : 'Aucune');
  ...
  tel.innerHTML = html;
  // Ajouter les onclick après injection
  var cards = tel.querySelectorAll('.cc');
  for (var i = 0; i < cards.length; i++) (function(card, idx){ card.onclick = function(){ openModal(t5[idx]); }; })(cards[i], i);
}
...
function renderColl() {
  ...
  fb.innerHTML = fhtml;
  // Stocker les sets pour setFilter
  window._sets = sets;
  ...
  // Ajouter onclick sur chaque carte (evite les problèmes d'encoding)
  var els = grid.querySelectorAll('.cc');
  for (var i = 0; i < els.length; i++) {
    (function(el, card) {
      el.onclick = function(e) { if (e.target.classList.contains('bdel')) return; openModal(card); };
    })(els[i], filtered[i]);
  }
}
...
function cardHTML(c, showDel) {
  ...
  return '<div class="cc">' +
    ...
    '<div class="cbd">' +
      ...
      '<div class="cu">💶 '+c.value.toFixed(2)+' € / unité</div>' +
      (c.qty > 1 ? '<div class="ctot">= '+(c.value*c.qty).toFixed(2)+' € total</div>' : '') +
...
function openModal(c) {
  ...
  // Stocker la carte courante pour applyModal
  window._modalCard = c;
  ov.innerHTML =
    '<div class="mb">'+
      '<div class="mh">'+
        ...
        '<div>'+
          ...
          '<span class="rar '+RCLS[ri]+'">'+esc(c.rarityLabel)+'</span>'+
          '<div style="margin-top:8px;font-size:.75rem;color:var(--t2)">Condition : <span style="color:var(--t)">'+esc(c.condition)+'</span></div>'+
        '</div>'+
      '</div>'+
      '<div class="mbody">'+
        '<div class="mp">'+
          '<div class="mpb"><div class="mpl">Cote unitaire</div><div class="mpv" style="color:var(--g)">'+c.value.toFixed(2)+' €</div></div>'+
          '<div class="mpb"><div class="mpl">Total ×'+c.qty+'</div><div class="mpv" style="color:var(--a)">'+(c.value*c.qty).toFixed(2)+' €</div></div>'+
        '</div>'+
        '<div class="mer">'+
          '<label>Cote (€)</label>'+
...
function mapRarity(r) {
  ...
  if (s.indexOf('ultra')>-1||s.indexOf('vmax')>-1||s.indexOf('full art')>-1||s.indexOf(' ex')>-1||s.indexOf('gx')>-1||s.indexOf('v ')>-1) return 3;
...
if (!collection.length) {
  collection = [
    {id:'1',name:'Dracaufeu ex',  set:'Écarlate et Violet',rarity:3,rarityLabel:'Ultra Rare', qty:1,value:45.00, condition:'Near Mint',img:'',addedAt:''},
    {id:'2',name:'Pikachu VMAX',  set:'Épée et Bouclier',  rarity:3,rarityLabel:'Ultra Rare', qty:2,value:28.50, condition:'Mint',     img:'',addedAt:''},
    {id:'3',name:'Mewtwo GX',     set:'Soleil et Lune',    rarity:2,rarityLabel:'Rare',        qty:1,value:12.00, condition:'Excellent',img:'',addedAt:''},
    {id:'4',name:'Ronflex',       set:'Écarlate et Violet',rarity:0,rarityLabel:'Commune',     qty:4,value:0.50,  condition:'Bon état', img:'',addedAt:''},
    {id:'5',name:'Lugia V',       set:'Argent Tempête',    rarity:4,rarityLabel:'Secrète',     qty:1,value:120.00,condition:'Mint',     img:'',addedAt:''}
  ];
  save();
}
renderDash();
